...
from kb.admin.learning_sections_callback import learning_sections_exercise_buttons, SectionsCd, all_exercise_cd, \
    SectionsId
        ...
        async with session.begin():
            ...
            if date_.month != datetime.today().month:
                await session.execute(
                    update(exercise_logs).where(exercise_logs.c.id == int(cur_exercise[0])).
                    values(is_done=is_done_, is_done_last_month=(not is_done_), date_change_status=datetime.now().date()))
            else:
                await session.execute(
                    update(exercise_logs).where(exercise_logs.c.id == int(cur_exercise[0])).
                    values(is_done=is_done_, date_change_status=datetime.now().date()))
            await session.commit()


@exercise_router.callback_query(F.data == select_done_cd, AdminState.admin_default)
@exercise_router.callback_query(F.data == select_done_cd, UserState.user_default)
...
@exercise_router.callback_query(F.data == select_undone_cd, AdminState.admin_default)
@exercise_router.callback_query(F.data == select_undone_cd, UserState.user_default)
...
async def select_exercise_all(callback: types.CallbackQuery, state: FSMContext):
    if callback.data == do_not_exercises_cd:
        await state.set_state(UserState.choose_section_undone)
    else:
        await state.set_state(UserState.choose_section_done)
    await callback.message.edit_text(text="Выберите тему", reply_markup=learning_sections_exercise_buttons())
    await callback.answer()


@exercise_router.callback_query(F.data == all_exercise_cd, UserState.choose_section_done)
@exercise_router.callback_query(F.data == SectionsCd.another_cd.value, UserState.choose_section_done)
@exercise_router.callback_query(F.data == SectionsCd.quality_cd.value, UserState.choose_section_done)
@exercise_router.callback_query(F.data == SectionsCd.meditation_cd.value, UserState.choose_section_done)
@exercise_router.callback_query(F.data == SectionsCd.method_stanislavsky_cd.value, UserState.choose_section_done)
@exercise_router.callback_query(F.data == SectionsCd.method_chekhov_cd.value, UserState.choose_section_done)
@exercise_router.callback_query(F.data == SectionsCd.method_demidov_cd.value,  UserState.choose_section_done)
@exercise_router.callback_query(F.data == SectionsCd.method_knebel_cd.value, UserState.choose_section_done)
async def select_exercise_all(callback: types.CallbackQuery, state: FSMContext):
    await state.set_state(UserState.get_exercise)
    async for session in get_async_session():
        async with session.begin():
            await state.update_data(is_list_undone=False)
            if callback.data != all_exercise_cd:
                section_id = SectionsId[callback.data].value
            else:
                section_id = -1

            exercises_list_ = await get_exercise_list(callback.message.chat.id, state, section_id)
...
@exercise_router.callback_query(F.data == all_exercise_cd, UserState.choose_section_undone)
@exercise_router.callback_query(F.data == SectionsCd.another_cd.value, UserState.choose_section_undone)
@exercise_router.callback_query(F.data == SectionsCd.quality_cd.value, UserState.choose_section_undone)
@exercise_router.callback_query(F.data == SectionsCd.meditation_cd.value, UserState.choose_section_undone)
@exercise_router.callback_query(F.data == SectionsCd.method_stanislavsky_cd.value, UserState.choose_section_undone)
@exercise_router.callback_query(F.data == SectionsCd.method_chekhov_cd.value, UserState.choose_section_undone)
@exercise_router.callback_query(F.data == SectionsCd.method_demidov_cd.value,  UserState.choose_section_undone)
@exercise_router.callback_query(F.data == SectionsCd.method_knebel_cd.value, UserState.choose_section_undone)
...
async def get_exercise_list(u_id: int, state: FSMContext, section_id: int):
    async for session in get_async_session():
        async with session.begin():
            if section_id == -1:
                ...
                if not ((await state.get_data())[is_list_undone]):
                    exercises = await session.execute(
                        select(exercise_logs).
                        where(exercise_logs.c.user_id == u_id).
                        where(exercise_logs.c.section_id == section_id).
                        order_by(exercise_logs.c.id))
                else:
                    exercises = await session.execute(
                        select(exercise_logs).
