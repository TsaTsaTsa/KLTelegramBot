...
async def delete_post_by_id(delete_post_id: int):
    async for session in get_async_session():
        async with session.begin():
            post_to_delete = await session.execute(posts.select().where(posts.c.id == delete_post_id))
            ...
            await session.execute(photos.delete().where(photos.c.post_id == delete_post_id))
            await session.execute(
                posts.delete().where(posts.c.id == delete_post_id))
            await session.commit()
