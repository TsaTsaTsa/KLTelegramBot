...
async def admin_add_text(callback: types.CallbackQuery, state: FSMContext):
    ...
    mes_to_delete = (await state.get_data())[msg_delete]
        ...
        async with session.begin():
            ...
            if new_post_text:
                result = await session.execute(
                    insert(posts).values(text=new_post_text, prev_post_id=prev_post_id, is_exercise=(await state.get_data())[is_ex]))
            else:
                result = await session.execute(
                    insert(posts).values(prev_post_id=prev_post_id, is_exercise=(await state.get_data())[is_ex]))

            await session.execute(update(users).where(users.c.next_post_id == None).values(next_post_id=result.inserted_primary_key[0]))

            await state.update_data(new_post_id=result.inserted_primary_key[0])
            if prev_post_id:
                await session.execute(
                    update(posts).where(posts.c.id == prev_post_id).values(next_post_id=result.inserted_primary_key[0]))
    ...
    async for session in get_async_session():
        async with session.begin():
            for new_photo in new_photos:
                await session.execute(insert(photos).values(file_id=new_photo, post_id=(await state.get_data())[new_post_id]))

            await session.execute(update(users).where(users.c.next_post_id is None).values(next_post_id=(await state.get_data())[new_post_id]))
            await session.commit()
    await delete_callback_list(messages)
...
async def end_create_post(msg: Message, state: FSMContext):
    ...
    await state.clear_data()
    await state.set_state(AdminState.admin_default)
    await msg.answer(text=admin.main_menu_text.text, reply_markup=main_menu.get_admin_main_menu_kb(
...
async def return_to_choose_item(callback: types.CallbackQuery, state: FSMContext):
    ...
    if len((await state.get_data())) == 0 or (await state.get_data())[new_post_id] == {}:
        if (await state.get_data())[is_ex]:
            await callback.message.answer(text="Ура-ура, новое упражнение!\n\n Давай попорядку: будет ли текст?",
                                          reply_markup=get_admin_new_post_menu_kb())
        else:
            await callback.message.answer(text=text.admin_add_content_text, reply_markup=get_admin_new_post_menu_kb())
        await callback.answer()
    else:
        async for session in get_async_session():
            async with session.begin():
                text_p = await session.execute(select(posts.c.text).where(posts.c.id == int((await state.get_data())[new_post_id])))
                await session.commit()
