...
async def delete_post(callback: types.CallbackQuery, state: FSMContext):
    callback_to_del = (await state.get_data())[callback_to_delete]
    await delete_callback_list(callback_to_del)
    print_post_to_delete_id = (await state.get_data())[print_post_id]
...
async def choose_post_to_update(callback: types.CallbackQuery, state: FSMContext):
...
async def choose_post_to_delete(msg: types.Message, state: FSMContext):
    ...
    if len(callback_id) == 0:
        ...
        callback_to_del = await append_mes_to_del(callback_id, callback_to_del)
        await state.update_data(callback_to_delete=callback_to_del)
